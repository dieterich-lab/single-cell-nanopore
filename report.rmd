...
library(ggplot2)
library(knitr)
names(args)=c('sim_barcodes','sim_label','sim_prob','real_prob','real_log','cutoff','sis_pred')
args = as.list(args)
...
# Simulated reads
## Benchmarking
```{r, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.height=4, fig.cap="\\label{fig:simbench}Performance metrics of the simulated reads"}
v = seq(0,100)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=5, fig.height=5, fig.cap="\\label{fig:realcount}Number of Nanopore reads each step"}
x=read.table(args$real_log,sep='\t')
a=system('wc -l analysis/real.label',intern=TRUE)
x[nrow(x)+1,]=c('Assigned to barcode', gsub('[^0-9]', '', a))
x[,2]=as.integer(x[,2])
x$Steps=factor(x[,1],levels=rev(x[,1]))
p=ggplot(x, aes(x=Steps, y=V2)) + geom_bar(stat="identity", position=position_dodge(), fill = "#ffff99") +coord_flip()+ labs(x="Processing steps", y="Number of simulated reads")+geom_text(aes(label=V2), position=position_dodge(width=0.9), hjust=1)
```
...
In Figure \ref{fig:realcomp} we compared the features and scores of the Nanopore reads which have either assigned to a barcode by ScNapBar, or Sicelore, or both. 
```{r, eval=FALSE, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, cache.lazy=FALSE, fig.width=8, fig.height=6, fig.cap="\\label{fig:realcomp}Features from the Nanopore reads"}
