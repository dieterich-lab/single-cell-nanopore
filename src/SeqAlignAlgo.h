...
public:
	...
	void alignGlobal(TAlignResults &a, flexbar::Alignments &alignments, flexbar::ComputeCycle &cycle, const unsigned int idxAl, const flexbar::TrimEnd trimEnd){
        ...
        int scoreStart = (trimEnd == ANY) ? a.startPos : a.startPosA;
        int scoreEnd = (trimEnd == ANY) ? a.endPos : a.endPosA;

		for(; it1 != end(row1); ++it1){

			if(scoreStart <= alPos && alPos < scoreEnd){
                if(*it1 == *it2 && *it2 != 'N')                                           ++matches;
                else if(isGap(it1))                                                       ++a.gapsR;
        ...
        if(!trimEnd == ANY){
            ...
            a.score = (0 < matches * mCost + (a.gapsR + a.gapsA) * gCost + a.mismatches * mmCost) ?
